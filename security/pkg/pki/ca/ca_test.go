...
func TestSignCSRForWorkload(t *testing.T) {
	...
	ca, err := createCA(time.Hour, false, false)
	if err != nil {
		t.Error(err)
	}
...
func TestSignCSRForProtectedWorkload(t *testing.T) {
	subjectID := "spiffe://example.com/ns/foo/sa/bar"
	opts := util.CertOptions{
		// This value is not used, instead, subjectID should be used in certificate.
		Host:       "spiffe://different.com/test",
	...
	ca, err := createCA(time.Hour, false, true)
	if err != nil {
		t.Error(err)
	}
...
func TestSignCSRForCA(t *testing.T) {
	...
	ca, err := createCA(365*24*time.Hour, true, false)
	if err != nil {
		t.Error(err)
	}
...
func TestSignCSRForProtectedCA(t *testing.T) {
	subjectID := "spiffe://example.com/ns/foo/sa/baz"
	opts := util.CertOptions{
		RSAKeySize: 2048,
		IsCA:       true,
	...
	ca, err := createCA(365*24*time.Hour, true, true)
	if err != nil {
		t.Error(err)
	}
...
func TestSignCSRTTLError(t *testing.T) {
	...
	ca, err := createCA(2*time.Hour, false, false)
	if err != nil {
		t.Error(err)
	}
...
// nolint: unparam
func createCA(maxTTL time.Duration, multicluster, protected bool) (*IstioCA, error) {
	...
	if protected {
		bl, _ := pem.Decode(intermediateKey)
		if bl == nil {
			return nil, nil
			//t.Fatalf("Failed to decode the private pem bytes: %v", err)
		}

		k, err := x509.ParsePKCS1PrivateKey(bl.Bytes)
		if err != nil {
			return nil, err
		}
		key = k
	} else {
		keyBytes = intermediateKey
	}

	bundle, err := util.NewVerifiedKeyCertBundleFromPem(
		intermediateCert, keyBytes, intermediateCert, rootCertBytes, key)
	if err != nil {
		return nil, err
	}
